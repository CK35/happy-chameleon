...
public class DeviceStatusUpdater {
    ...
    public DeviceStatusUpdater(List<SensorUpdater> tasks) {
        LOG.debug("Created device status updater for: {} tasks.", tasks.size());
        this.tasks = tasks;
    }
    ...
    public static class DigitalPinStateUpdater implements SensorUpdater {
        ...
        @Override
        public void update() {
            sensor.pushValue(pin.getState()
                                .isHigh());
        }
    }

    public static class Errors implements ErrorHandler, RejectedExecutionHandler {
